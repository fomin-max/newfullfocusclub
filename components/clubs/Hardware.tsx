...
function iconFor(k: string): IconName {
  if (/Видеокарта/i.test(k))        return 'monitor'
  if (/Процессор/i.test(k))         return 'cpu'
  if (/RAM|ОЗУ/i.test(k))           return 'memory'
  if (/Коврик/i.test(k))            return 'keyboard'
  if (/Монитор/i.test(k))           return 'monitor'
  if (/Проектор/i.test(k))          return 'projector'
  if (/Кресло|Посадка/i.test(k))    return 'sofa'
  if (/Клавиатура/i.test(k))        return 'keyboard'
  if (/Мышка|Мышь/i.test(k))        return 'mouse'
  if (/Наушники/i.test(k))          return 'headphones'
